...
```{r, include = FALSE}
#Define colors 
emp_ind_vector <- c(brewer.pal(9, "Greys")[4], "#cf4633", "#BEAED4", "#E3AC78", "#FDBF6F",  "#386CB0", "#FB8072", brewer.pal(9, "Greys")[3], brewer.pal(9, "Greys")[8], "#80B1D3", "#F0027F",  "#4DAF4A", "#F1E2CC", "#6A3D9A", "#E78AC3", "#CBD5E8", "#666666", brewer.pal(9, "Greys")[5],  brewer.pal(9, "Greys")[6], "#A65628", brewer.pal(9, "Greys")[7])
```
...
```{r}
final_cbp_results <- readRDS(here("Final Data/final_cbp_io_results_nohhi_supply_cor.RDS"))
```
...
vert_emp_estabs <- vert_agg %>% 
  ggplot(aes(x = vert_dist_emp, y = vert_dist_estabs, fill = naics_3digit_label, text = industry_desc)) +
  geom_point(shape = 21) +
  scale_fill_manual(values = emp_ind_vector) + 
  guides(fill = "none") + 
  theme_bw() + 
...
```{r}
county_horz_agg_df <- final_results_merge %>% 
  county_sum_data(output_emp, output_emp_mean, output_emp_sd) %>% 
  rename(horz_emp_mean = agg_var_mean, horz_emp_sd = agg_var_sd, horz_max = agg_max, horz_dist = mean_dist, sd_horz = sd_agg, horz_ind = agg_ind)

county_vert_agg_df <- final_results_merge %>% 
  county_sum_data(supply_emp, supply_emp_mean, supply_emp_sd) %>% 
  rename(vert_emp_mean = agg_var_mean, vert_emp_sd = agg_var_sd, vert_max = agg_max, vert_dist = mean_dist, sd_vert = sd_agg, vert_ind = agg_ind)

county_agg <- full_join(county_horz_agg_df, county_vert_agg_df) %>% 
  left_join(final_results_merge %>% select(area_fips, NAME) %>% distinct()) %>% filter(!is.na(horz_emp_mean), !is.na(vert_estabs_mean))
```
...
all_agg_map <- county_agg_map_df %>% 
  ...
  geom_polygon(data = county_agg_map_df %>% filter(agg_quad == "General Agglomeration"), aes(fill = agg_dist),  color = "#012898", show.legend = FALSE) +
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) + 
  scale_fill_gradient(low = "#f6f1eb", high = "#012898", limits =  c(0.03, 4.43)) + 
  new_scale_fill() + 
  geom_polygon(data = county_agg_map_df %>% filter(agg_quad == "Supplier Agglomeration"), aes(fill = agg_dist), color = "#E3A902", show.legend = FALSE) +
  scale_fill_gradient(low = "#f6f1eb", high = "#E3A902", limits =  c(0.02, 1.80)) + 
  new_scale_fill() + 
  geom_polygon(data = county_agg_map_df %>% filter(agg_quad == "Peer Agglomeration"), aes(fill = agg_dist),  color = "#019826", show.legend = FALSE) +
  scale_fill_gradient(low = "#f6f1eb", high = "#019826", limits =  c(0.05, 3.99)) + 
  new_scale_fill() + 
  geom_polygon(data = county_agg_map_df %>% filter(agg_quad == "Low Agglomeration"), aes(fill = agg_dist),  color = "#8A836D", show.legend = FALSE) +
  scale_fill_gradient(low = "#f6f1eb", high = "#8A836D", limits =  c(0.01, 5.91)) +  
  theme_map() 
...
map_function <- function(df){
  ...
  scale_fill_gradient(low = "#f6f1eb", high = "#049BCB", limits =  c(0, 5)) + 
  new_scale_fill() + 
...
```
```{r}
sector_facet <- sector_var + facet_wrap(~naics_3digit_label)
```
...
```{r}
focus_graph_points <- focus_graph + geom_point(data = focus_sector_var %>% filter(ind_code %in% detailed_ind_list), aes(x = horz_dist_estabs, y = vert_dist_estabs), fill = "grey", size = 5, shape = 21)
```
...
```{r}
all_counties_industries <- final_results_merge %>% 
  filter(supply_estabs > 0, output_estabs > 0) %>% 
  mutate(horz_agg = log(output_estabs), 
         vert_agg = log(supply_estabs)) %>% 
  normalize(horz_agg) %>% 
  select(-c(var_mean, var_sd)) %>% 
  rename(horz_agg_norm = norm_var) %>% 
  normalize(vert_agg) %>% 
  select(-c(var_mean, var_sd)) %>% 
  rename(vert_agg_norm = norm_var) 
```

```{r}
all <- all_counties_industries %>% 
  ggplot() + 
  geom_point(aes(x = horz_agg_norm, y = vert_agg_norm, fill = naics_3digit_label), shape = 21, size = 3, alpha = 0.9) + 
    scale_fill_manual(values = emp_ind_vector)+ 
    guides(fill = "none") + 
  geom_abline(slope = 1, intercept = 0) + 
  geom_abline(slope = 1, intercept = 1, linetype = 2) + geom_abline(slope = 1, intercept = -1, linetype = 2) + 
   labs(x = "Industry Output Horizontal Agglomeration Z-Score\nEstablishments, County X Industry", y = "Industry Vertical Agglomeration Z-Score\nEstablishments, County X Industry") + 
  theme_bw() + 
  axis_theme
```
...
```{r}
all + facet_wrap(~naics_3digit_label)
```

```{r}
corner_inds <- all_counties_industries %>% 
  filter(ind_code %in% c("324110", "3219A0", "322210", "332114", "332430", "334111", "334413", "334210", "325411", "336992", "336411", "336111"))

corner_cols <- get_col_match(corner_inds)

corner_graph <- corner_inds %>% 
  mutate(industry_desc = gsub("[[:punct:]]", " ", industry_desc), 
         industry_desc = case_when(
           str_detect(industry_desc, "metal") ~ "Metal can & box, light guage", 
           str_detect(industry_desc, "tank") ~ "Military Tank and Tank Components", 
           TRUE ~ industry_desc
         )) %>% 
  ggplot() + 
  geom_point(aes(x = horz_agg_norm, y = vert_agg_norm, fill = naics_3digit_label), shape = 21, size = 3, alpha = 0.9) + 
...
horz_rd <- op_horz %>% 
  ...
  ggplot(aes(x = rd_sup_perc, y = horz_dist_estabs, text = industry_desc)) + 
...
vert_rd <- vert_agg %>% 
  ...
  mutate(rd_sup_perc = log(rd_sup_perc)) %>% 
  normalize(rd_sup_perc) %>% 
  ggplot(aes(x = norm_var, y = vert_dist_estabs, text = industry_desc)) + 
  geom_point(aes(fill = naics_3digit_label), shape = 21, size = 3) +
  geom_smooth(color = "dark grey", method = "lm") + 
  scale_fill_manual(values = emp_ind_vector) + 
  guides(fill = "none")  + 
...
horz_mat <- op_horz %>% 
  ...
  ggplot(aes(x = manf_only, y = horz_dist_estabs, text = industry_desc)) + 
  geom_point(aes(fill = naics_3digit_label), shape = 21, size = 3) +
  geom_smooth(color = "dark grey", method = "lm") + 
  scale_fill_manual(values = emp_ind_vector)+ 
  guides(fill = "none") + 
  ylim(c(-5, 4.1)) + 
  theme_bw() + 
  labs(x = "Industry Manufactured Inputs Share of \nTotal Value of Intermediate Inputs", y = "Industry Output Horizontal Agglomeration Z-Score\nEstablishments, Mean Over Counties") + 
  axis_theme
```
...
vert_mat <- vert_agg %>% 
  left_join(mat_intensity) %>% 
  ggplot(aes(x = manf_only, y = vert_dist_estabs, text = industry_desc)) + 
  geom_point(aes(fill = naics_3digit_label), shape = 21, size = 3) + 
  geom_smooth(color = "dark grey", method = "lm") + 
  scale_fill_manual(values = emp_ind_vector)+ 
  guides(fill = "none") + 
  ylim(c(-5, 4.1)) + 
  theme_bw() + 
  labs(x = "Industry Manufactured Inputs Share of \nTotal Value of Intermediate Inputs", y = "Industry Vertical Agglomeration Z-Score\nEstablishments, Mean Over Counties") + 
  axis_theme
```
...
horz_mat1 <- op_horz %>% 
  left_join(mat_intensity) %>% 
  ggplot(aes(x = mats_manf, y = horz_dist_estabs)) + 
  geom_point(aes(fill = naics_3digit_label), shape = 21, size = 3) +
  geom_smooth(color = "dark grey", method = "lm") + 
  scale_fill_manual(values = emp_ind_vector)+ 
  guides(fill = "none") + 
...
vert_mat1 <- vert_agg %>% 
  left_join(mat_intensity) %>% 
  ggplot(aes(x = mats_manf, y = vert_dist_estabs)) + 
  geom_point(aes(fill = naics_3digit_label), shape = 21, size = 3) + 
  geom_smooth(color = "dark grey", method = "lm") + 
  scale_fill_manual(values = emp_ind_vector)+ 
  guides(fill = "none") + 
...
val_add <- vert_agg %>% 
  left_join(op_horz) %>% 
  left_join(ind_agg %>% rename(ind_code = industry_code)) %>% 
  mutate(log_val = log(value_added))
```

```{r}
horz_add <- val_add %>% 
  ggplot(aes(x = log_val, y = horz_dist_estabs)) + 
  geom_point(aes(fill = naics_3digit_label), shape = 21, size = 3) +
  geom_smooth(color = "dark grey", method = "lm") + 
  scale_fill_manual(values = emp_ind_vector)+ 
  guides(fill = "none") + 
  ylim(c(-5, 4.1)) + 
  theme_bw() + 
  labs(x = "Log of Value Added \n Industry Level", y = "Industry Output Horizontal Agglomeration Z-Score\nEstablishments, Mean Over Counties") + 
  axis_theme
```

```{r}
vert_add <- val_add %>% 
  ggplot(aes(x = log_val, y = vert_dist_estabs)) + 
  geom_point(aes(fill = naics_3digit_label), shape = 21, size = 3) +
  geom_smooth(color = "dark grey", method = "lm") + 
  scale_fill_manual(values = emp_ind_vector)+ 
  guides(fill = "none") + 
  ylim(c(-5, 4.1)) + 
  theme_bw() + 
  labs(x = "Log of Value Added \n Industry Level", y = "Industry Vertical Agglomeration Z-Score\nEstablishments, Mean Over Counties") + 
  axis_theme
```
...
```{r}
final_data_large <- final_results_merge %>% 
  filter(output_estabs > 0, supply_estabs > 0) %>%  
  mutate(horz_agg = log(output_estabs), vert_agg = log(supply_estabs)) %>% 
  normalize(horz_agg) %>% 
  select(-c(var_mean, var_sd)) %>% rename(horz_z = norm_var) %>% 
  normalize(vert_agg) %>% 
  select(-c(var_mean, var_sd)) %>% rename(vert_z = norm_var) %>% 
  left_join(rd_output) %>% 
  mutate(rd_sup_perc = log(rd_sup_perc)) %>% 
  normalize(rd_sup_perc) %>% rename(rd_z = norm_var) %>% 
  select(-c(var_mean, var_sd)) %>%  left_join(mat_intensity) %>% 
  left_join(us_pop %>% filter(variable == "DENSITY")) %>% 
  left_join(ind_agg %>% rename(ind_code = industry_code)) 
```
...
```{r}
final_data <- op_horz %>% left_join(vert_agg) %>% left_join(final_results_merge %>% select(ind_code, ind_tot_emp_nat, ind_tot_estab_nat) %>% distinct()) %>%  left_join(rd_output) %>% mutate(log_rd_sup = log(rd_sup_perc)) %>% normalize(log_rd_sup) %>% rename(rd_z = norm_var) %>% select(-c(var_mean, var_sd)) %>%  left_join(mat_intensity) 
```

```{r}
final_data_all <- final_data_large %>% left_join(cbp_all) %>% mutate(s_ri_estabs = county_ind_tot_estabs / county_tot_estabs, 
                                                   s_ri_emp = county_ind_tot_emp / county_tot_emp) %>% 
    #REST OF EGK MEASURES (X_RI)
    group_by(NAME) %>% 
...
```{r}
horz_rd_lm_long <- lm(horz_z ~ log(ind_tot_emp_nat) + log(x_r_emp)  + rd_z + manf_only, data = final_data_all)
```

```{r}
vert_rd_lm_long <- lm(vert_z ~ log(ind_tot_emp_nat) + log(x_r_emp) + rd_z + manf_only, data = final_data_all)
```

```{r}
vert_lm_cont_ind_county <- lm(vert_z ~ log(ind_tot_emp_nat) +  log(x_r_emp) + rd_z + manf_z + horz_z, data = final_data_all)
```

```{r}
horz_lm_cont_ind_county <- lm(horz_z ~ log(ind_tot_emp_nat) + log(x_r_emp) + rd_z + manf_z + vert_z, data = final_data_all)
```

```{r}
export_summs(horz_lm_cont_ind_county, vert_lm_cont_ind_county)
```


```{r}
vert_lm_cont_ind_val <- lm(vert_z ~ log(ind_tot_emp_nat) +  log(x_r_emp) + rd_z + log(manf_only) + horz_z + log_val, data = final_data_all)
```

```{r}
horz_lm_cont_ind_val <- lm(horz_z ~ log(ind_tot_emp_nat) + log(x_r_emp) + rd_z + log(manf_only) + vert_z + log_val, data = final_data_all)
```

```{r}
vert_lm_cont_ind_val_int <- lm(vert_z ~ log(ind_tot_emp_nat) +  log(x_r_emp) + rd_z + log(manf_only) + horz_z + log_val + log_val*log(manf_only), data = final_data_all)
```

```{r}
horz_lm_cont_ind_val_int <- lm(horz_z ~ log(ind_tot_emp_nat) + log(x_r_emp) + rd_z + log(manf_only) + vert_z + log_val + log_val*log(manf_only), data = final_data_all)
```

```{r}
export_summs(horz_rd_lm_long, vert_rd_lm_long, horz_lm_cont_ind_county, vert_lm_cont_ind_county)

```


```{r}
export_summs(horz_lm_cont_ind_val, vert_lm_cont_ind_val, horz_lm_cont_ind_val_int, vert_lm_cont_ind_val_int)
```


```{r}
vert_lm <- lm(vert_dist_estabs ~ log(ind_tot_emp_nat)  + rd_z + manf_only, data = final_data)
```

```{r}
horz_lm <- lm(horz_dist_estabs ~ log(ind_tot_emp_nat)  + rd_z + manf_only, data = final_data)
```

```{r}
vert_lm_cont <- lm(vert_dist_estabs ~ log(ind_tot_emp_nat)  + rd_z + manf_only + horz_dist_estabs, data = final_data)
```

```{r}
horz_lm_cont <- lm(horz_dist_estabs ~ log(ind_tot_emp_nat)  + rd_z + manf_only + vert_dist_estabs, data = final_data)
```

```{r}
export_summs(horz_lm, vert_lm, horz_lm_cont, vert_lm_cont)
```
...
```{r}
rd_dist <- final_data %>% 
  ggplot(aes(x = rd_z, y = reorder(naics_3digit_label, rd_z), fill = naics_3digit_label)) + 
  geom_density_ridges() + 
  geom_point(shape = 21) +
  scale_fill_manual(values = emp_ind_vector) + 
  guides(fill = "none") + 
  labs(x = "Industry R&D Intensity Z-Score", y = "NAICS 3-Digit Label") +
  theme_bw() +
  axis_theme + 
...
```{r}
mat_dist <- final_data %>% 
  ggplot(aes(x = manf_only, y = reorder(naics_3digit_label, rd_z), fill = naics_3digit_label)) + 
  geom_density_ridges() + 
  geom_point(shape = 21) +
  scale_fill_manual(values = emp_ind_vector) + 
  guides(fill = "none") + 
  labs(x = "Share of Value of Intermediate Inputs\n from Manufacturing", y = "") +
  theme_bw() +
...
```{r}
emp_dist <- final_data %>% 
  ggplot(aes(x = log(ind_tot_emp_nat), y = reorder(naics_3digit_label, rd_z), fill = naics_3digit_label)) + 
  geom_density_ridges() + 
  geom_point(shape = 21) +
  scale_fill_manual(values = emp_ind_vector) + 
  guides(fill = "none") + 
  labs(x = "Log Industry Total Employment (2019)", y = "") +
  theme_bw() +
  theme(axis.text.y = element_blank(), 
        axis.title.x = element_text(size = 12))
```
...
vert_pay <- vert_agg %>% 
  ...
  axis_theme
```
## R&D Intensity V. Materials Intensity

```{r}
trim_data <- final_data %>% 
  left_join(ind_crosswalk) %>% 
  filter(ind_code %in% c(detailed_ind_list, "327320")) %>% 
  select(rd_z, manf_only, ind_code, naics_3digit_label, industry_desc) %>% 
  distinct()
```
...
rd_mats <- final_data %>% 
  left_join(ind_crosswalk) %>% 
  mutate(log_manf = log(manf_only)) %>% 
  normalize(log_manf) %>% 
  ggplot(aes(x = rd_z, y = norm_var, label = industry_desc)) + 
  geom_point(aes(fill = naics_3digit_label), shape = 21, size = 5, alpha = 0.9) + 
  scale_fill_manual(values = emp_ind_vector) + 
  guides(fill = "none") + 
  labs(x = "RD Intensity Z-Score", y = "Manufactured Share of Intermediate Inputs") + 
  theme_bw() + 
  axis_theme
```
...
```{r}
focus_means <- focus_ind_counties + geom_point(data = focus_sector_var %>% filter(ind_code %in% detailed_ind_list) %>% left_join(final_results_merge %>% select(ind_code, NAME, ind_share_emp)), aes(x = horz_dist_estabs, y = vert_dist_estabs), fill = "grey", shape = 21, size = 5) 
```
...
```{r}
semi_counties <- all_agg %>% 
  filter(ind_code %in% c("334413")) %>% 
  mutate(semi_horz = mean(horz_dist), 
         semi_vert = mean(vert_dist))
```

```{r}
focus_counties <- c("Maricopa County, Arizona", "Washington County, Oregon", "Dallas County, Texas")
```

```{r}
semi_col <- get_col_match(semi_counties)
```


```{r}
semi_graph <- semi_counties %>% 
  ggplot(aes(text = NAME)) + 
  geom_point(aes(x = horz_dist, vert_dist), fill = semi_col, shape = 21, size = 5) + 
  geom_label_repel(data = semi_counties %>% filter(NAME %in% focus_counties), aes(x = horz_dist, vert_dist, label = NAME), fill = "lightgrey", size = 5) + 
  geom_point(data = semi_counties %>% filter(NAME %in% focus_counties), aes(x = horz_dist, vert_dist), fill = "lightgrey", shape = 21, size = 5) + 
  # geom_vline(xintercept = semi_counties$semi_horz, color = semi_col) + 
  # geom_hline(yintercept = semi_counties$semi_vert, color = semi_col) + 
  geom_vline(xintercept = 0) + 
  geom_hline(yintercept = 0) + 
  labs(x = "County-Industry Horizontal Agglomeration Z-Score", y = "County-Industry Vertical Agglomeration Z-Score") + 
  theme_bw() + 
  axis_theme
...
```{r}
concrete_map <- map_function(county_agg_map_df %>% 
               filter(NAME %in% get_counties(final_results_merge, "327320")))
```
...
```{r}
rd_output %>% 
  mutate(rd_sup_perc = 100*rd_sup_perc) %>% 
