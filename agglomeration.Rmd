...
```{r}
cor_fn <- function(matrix){
  
  as.dist(1-cor(t(matrix), method = c("pearson"))) %>% 
    as.matrix()
...
```{r}
results_ind_emp <- results %>% 
  filter(dist_mi < 100, !is.na(industry_code)) %>% 
  select(industry_code, NAME, ind_a_gi_emp, ind_a_gi_emp) %>% 
  distinct() %>% 
  pivot_wider(id_cols = industry_code, names_from = NAME, values_from = ind_a_gi_emp, values_fill = 0) %>% 
  select(-c(industry_code)) %>% 
  as.matrix()

results_ind_estabs <- results %>% 
  filter(dist_mi < 100, !is.na(industry_code)) %>% 
  select(industry_code, NAME, ind_a_gi_estabs, ind_a_gi_estabs) %>% 
  distinct() %>% 
  pivot_wider(id_cols = industry_code, names_from = NAME, values_from = ind_a_gi_estabs, values_fill = 0) %>% 
  select(-c(industry_code)) %>% 
  as.matrix()
```

Using the initial list of industries from earlier, I obtain a set of industry relatedness measures from the input output data, focusing on how industries are related to the industry of interest, $i$. 

```{r}
#Use list of industries to get index of industries in relatedness measures
  ind_list_index <- which(ind_list %in% geog_ind)

  ind_i <- which(ind_list %in% ind_in_county)
  ...
  horz_agg <- Y_Pi[ind_i, ind_list_index] %>% as.matrix()
  
  vert_agg_1 <- M_ij[ind_i, ind_list_index] %>% as.matrix()
  
  vert_agg_2 <- X_Ci[ind_i, ind_list_index] %>% as.matrix()
```


For each relatedness matrix, I multiply by the geographic matrix defined in the above section.

The resulting matrix $\beta_{j,g}$ then has rows defined by the industries related to the target industry $i$, across the counties that are within a specific geographic distance of that county. An individual entry in this matrix is the sum of the geographically weighted correlation between the individual industry $i$ and all other industries in a given county g that falls within some radius of the initial county of interest. 

```{r}
county_activity_norm <- length(ind_list_index) - proxyC::colZeros(results_ind_emp) 
```

```{r}
duplicated_matrix <- sapply(1:length(ind_i), FUN = function(i) rep(county_activity_norm, 1))

county_norm <- duplicated_matrix %>% t()
```

```{r}
final_matrix_calc <- function(matrix1, matrix2){
  
  result1 <- matrix1 %*% matrix2
  
  final_result <- result1 / county_norm
}
```

While the full matrix might be useful in the future, at the moment, I am interested in obtaining single point estimates for county-sub-sector entities. As such, I average over all the counties within the radius. 

```{r}
  horz_emp <- final_matrix_calc(horz_agg, results_ind_emp)
  horz_estab <- final_matrix_calc(horz_agg, results_ind_estabs)
  
  vert1_emp <- final_matrix_calc(vert_agg_1, results_ind_emp)
  vert1_estab <- final_matrix_calc(vert_agg_1, results_ind_estabs)
  
  vert2_emp <- final_matrix_calc(vert_agg_2, results_ind_emp)
  vert2_estab <- final_matrix_calc(vert_agg_2, results_ind_estabs)
```
...
```{r}
county_ind$horz_emp_mean <- rowMeans(horz_emp)
county_ind$horz_estab_mean <- rowMeans(horz_estab)
county_ind$vert1_emp_mean <- rowMeans(vert1_emp)
county_ind$vert1_estab_mean <- rowMeans(vert1_estab)
county_ind$vert2_emp_mean <- rowMeans(vert2_emp)
county_ind$vert2_estab_mean <- rowMeans(vert2_estab)
```
