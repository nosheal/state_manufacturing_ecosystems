...
```{r}
mining_col <- data.frame(ind_3digit.naics_3digit_label = "Mining, quarrying, and oil and gas extraction", emp_ind_vector = "#000000")
```

```{r}
col_map <- bind_rows(col_map, mining_col)
```
...
With these data, our preliminary, and primary, analysis will be focused around county-level employment by subsector. 

### Cleaning Estimates of Establishments and Employees

Because details about establishment employment are suppressed when there are only a few establishments, we first need to create a measure for estimating the number of employees at an establishment based on the type of establishment that it is. Following Delgado et al., 2014, we use the mid-point for an establishment employee range as our estimate of the number of employees at that establishment.
...
More concerning, we see that the sum of the establishments across various employee sizes does not always equal the "all establishments" size category total number of establishments, in sometimes concerning ways. For example, steel manufacturing in Allegheny County does not appear in our data, when just looking at the distribution of establishments across employee sizes, and 5-digit industries. 

```{r}
#CREATE ESTIMATE OF TOTAL ESTABLISHMENTS AND EMPLOYEES IN COUNTY, INDUSTRY 
all_estabs_est <- cbp_2019 %>% 
  group_by(NAICS2017_LABEL, NAME) %>% 
  filter(EMPSZES != 1) %>% 
  reframe(all_estabs = sum(ESTAB), all_emp = sum(emp_tot))
```

We can examine the distribution of the difference in establishment and employee counts by industry code, to see if certain industries are underestimated. 


```{r}
#Compare against data on all establishments and employees
estab_comp_all <- cbp_2019 %>% 
  filter(EMPSZES == 1) %>% 
  left_join(all_estabs_est) %>% 
  select(naics_3digit_label, NAME, NAICS2017, NAICS2017_LABEL, ESTAB, EMP, all_estabs, all_emp) %>% 
   mutate(emp = as.numeric(EMP), 
     estab_diff = ESTAB - all_estabs, 
         emp_diff = emp - all_emp) %>% 
   mutate(estab_diff = case_when(
    is.na(estab_diff) ~ ESTAB, 
    TRUE ~ estab_diff
  ), 
  emp_diff = case_when(
    is.na(emp_diff) ~ emp, 
    TRUE ~ emp_diff), 
 abs_emp_diff = case_when(
    is.na(emp_diff) ~ emp, 
    TRUE ~ abs(emp_diff)
  ) )
```
...
comp_graph <- estab_comp_all %>% 
  mutate(naics_3digit_label = str_remove(naics_3digit_label, "manufacturing")) %>% 
  group_by(naics_3digit_label) %>% 
  mutate(mean_diff = mean(estab_diff, na.rm = TRUE)) %>% 
  ggplot() + 
  geom_density_ridges(aes(x = estab_diff, y = reorder(naics_3digit_label, mean_diff), fill = naics_3digit_label)) + 
  scale_fill_manual(values = emp_ind_vector) + 
  guides(fill = "none") + 
  labs(x = "Difference between total establishments and all establishments", y = "NAICS 3-Digit Code") + 
  theme_bw() +
  axis_theme
...
We see that on average, the difference in number of establishments is around 4, and that the pattern across is industries is very even, and tightly distributed, suggesting that there is systematic noise involved. 

When comparing across total number of employees (assuming that we can ignore employee differences in cases where there is no difference in establishment counts), we see that we are missing a substantial amount of employment, roughly evenly across industries. 

```{r, fig.width=11,fig.height=8}
emp_comp <- estab_comp_all %>% 
  filter(estab_diff != 0, abs_emp_diff > 0 ) %>% 
  mutate(naics_3digit_label = str_remove(naics_3digit_label, "manufacturing")) %>% 
  group_by(naics_3digit_label) %>% 
  mutate(mean_diff = mean(abs_emp_diff, na.rm = TRUE)) %>% 
  ggplot() + 
  geom_density_ridges(aes(x = log(abs_emp_diff), y = reorder(naics_3digit_label, mean_diff), fill = naics_3digit_label)) + 
  scale_fill_manual(values = emp_ind_vector) + 
  guides(fill = "none") + 
  labs(x = "Log Absolute Difference between total estimated employees \nand all establishment employees", y = "NAICS 3-Digit Code") + 
  theme_bw() +
  axis_theme
...
We do not know how employees are distributed across establishments when they are accounted for in the "all establishment"  size category, rather than by a specific size of establishment.

At this stage, we are particularly interested in ensuring that we account for the presence of certain industries that would otherwise be missed, likely due to data supression. 

As such, for our corrections, we make a few critical assumptions. 1) We only "add" back data for cases where a specific industry would not have appeared as present in a location without including all establishments. 

```{r}
estab_comp_new <- cbp_2019 %>% 
  group_by(NAME, NAICS2017_LABEL) %>% 
  mutate(emp_count = n()) %>% 
  filter(EMPSZES == 1, emp_count == 1) %>% 
  select(NAME, NAICS2017_LABEL) %>% 
  distinct() %>% 
  left_join(estab_comp_all)
```


Secondly, for these establishments, we treat employees as distributed evenly across the number of establishments. While this is a coarse correction it allows us to capture the presence of certain very large firms, such as US Steel in Allegheny County. To refine this, we could consider weighting the correction by the average number of employees for a specific industry. 

Here, inaccuracies in these data reinforce the need to perform this analysis again with NETS data. 

```{r}
cbp_correction <- estab_comp_new %>% 
  filter(estab_diff != 0,
         emp_diff > 0) %>% 
  mutate(emp_diff_raw = emp_diff, 
         emp_diff = round(emp_diff / estab_diff),
    EMPSZES = case_when(
  ...
  select(naics_3digit_label, NAME, NAICS2017_LABEL, EMPSZES, ESTAB = estab_diff, EMP = emp_diff, emp_tot = emp_diff_raw) %>% 
  left_join(cbp_2019 %>% 
              select(NAME, NAICS2017_LABEL, area_fips, STATEFP, COUNTYFP, st, state_abbr, GEO_ID) %>% 
              distinct()) %>% 
  left_join(cbp_2019 %>% 
              select(EMPSZES, EMPSZES_LABEL) %>% 
              distinct()) %>% 
  mutate(EMP = as.character(EMP), 
         EMPSZES_LABEL = paste("EST.", EMPSZES_LABEL, sep = ""))
```

```{r}
cbp_2019_clean <- bind_rows(cbp_2019, cbp_correction)
```

We can now proceed with our corrected data, and will also endeavor to display the total number of employees estimated to belong to a specific entity (defined currently as a county-5digit-establishment_size combination). 
...
label_2 <- function(data, var, condition){
  ...
  sprintf("<strong>%s</strong><br/>%s <br/>%s <br/> Total Establishments: %s<br/> Estimated Employees: %s",
...
```{r, fig.width=11,fig.height=8}
v_large_col <- get_col_match(cbp_2019_clean, 260)


v_large_graph <- cbp_2019_clean %>% 
  ...
  theme_bw() + 
  labs(x = "Total Number of Establishments", y = "5 Digit NAICS Label", fill = "3-Digit NAICS Label", title = "Very Large Establishments (1,000+ Employees)", subtitle = title_fun(cbp_2019_clean, 260)) + 
  axis_theme + 
...
```{r, fig.width=11,fig.height=8}
v_large_map <- estab_size_circles(manf_share_clean, cbp_2019_clean, 260, EMPSZES_LABEL, rad = 150000, fill_opac = .8)
...
```{r, fig.width=11,fig.height=8}
large_col <- get_col_match(cbp_2019_clean, 254)

large_graph <- cbp_2019_clean %>% 
  ...
  scale_y_discrete(labels = scales::label_wrap(50)) + 
  theme_bw() + 
  labs(x = "Total Number of Establishments", y = "5 Digit NAICS Label", fill = "3-Digit NAICS Label", title = "Large Establishments (500-999 Employees)", subtitle = title_fun(cbp_2019_clean, 254)) + 
  axis_theme + 
  theme(legend.position = "bottom")
...
```{r, fig.width=11,fig.height=8}
large_map <- estab_size_circles(manf_share_clean, cbp_2019_clean, 254, EMPSZES_LABEL, rad = 75000, fill_opac = .9)
...
```{r, fig.width=11,fig.height=8}
large_map_overlap <- estab_size_circles(v_large_map, cbp_2019_clean, 254, EMPSZES_LABEL, rad = 75000, fill_opac = .9)
...
```{r, fig.width=11,fig.height=8}
med_large_col <- get_col_match(cbp_2019_clean, 252)

med_large_graph <- cbp_2019_clean %>% 
  ...
  theme_bw() + 
  labs(x = "Total Number of Establishments", y = "5 Digit NAICS Label", fill = "3-Digit NAICS Label", title = "Medium Large Establishments (250-499)", subtitle = title_fun(cbp_2019_clean, 252)) + 
  theme(legend.position = "bottom")
...
```{r, fig.width=11,fig.height=8}
med_large_map <- estab_size_circles(manf_share_clean, cbp_2019_clean, 252, EMPSZES_LABEL, 37500, .9, color_opac = 0.5, border_size = 1)
...
```{r, fig.width=11,fig.height=8}
med_large_map_overlap_full <- estab_size_circles(large_map_overlap, cbp_2019_clean, 252, EMPSZES_LABEL, 37500, .9, color_opac = 0.5, border_size = 1)
...
```{r, fig.width=11,fig.height=8}
med_col <- get_col_match(cbp_2019_clean, 251)

med_graph <- cbp_2019_clean %>% 
  ...
  scale_y_discrete(labels = scales::label_wrap(50), guide = guide_axis(n.dodge=2)) + 
  theme_bw() + 
  labs(x = "Total Number of Establishments", y = "5 Digit NAICS Label", fill = "3-Digit NAICS Label", title = "Medium Establishments (100-250 employees)", subtitle = title_fun(cbp_2019_clean, 251)) + 
  theme(legend.position = "bottom")
...
```{r, fig.width=11,fig.height=8}
med_map <- estab_size_circles(manf_share_clean, cbp_2019_clean, 251, EMPSZES_LABEL, 17500, .9, color_opac = 0.5, border_size = 1)
...
```{r, fig.width=11,fig.height=8}
med_map_overlap_full <- estab_size_circles(med_large_map_overlap_full, cbp_2019_clean, 251, EMPSZES_LABEL, 17500, .9, color_opac = 0.5, border_size = 1)
...
```{r, fig.width=11,fig.height=8}
med_small_col <- get_col_match(cbp_2019_clean, 242)

med_small_graph <- cbp_2019_clean %>% 
  ...
  scale_y_discrete(labels = scales::label_wrap(50), guide = guide_axis(n.dodge=2)) + 
  theme_bw() + 
  labs(x = "Total Number of Establishments", y = "5 Digit NAICS Label", fill = "3-Digit NAICS Label", title = "Medium-Small Establishments (50-99 employees)", subtitle = title_fun(cbp_2019_clean, 242)) + 
  theme(legend.position = "bottom")
...
```{r, fig.width=11,fig.height=8}
med_small_map <- estab_size_circles(manf_share_clean, cbp_2019_clean, 242, EMPSZES_LABEL, 8750, .9, color_opac = 0.5, border_size = 0.1)
...
```{r, fig.width=11,fig.height=8}
med_small_overlap_full <- estab_size_circles(med_map_overlap_full, cbp_2019_clean, 242, EMPSZES_LABEL, 8750, .9, color_opac = 0.5, border_size = 0.1)
...
```{r, fig.width=11,fig.height=8}
small_med_col <- get_col_match(cbp_2019_clean, 241)

small_med_graph <- cbp_2019_clean %>% 
  ...
  scale_y_discrete(labels = scales::label_wrap(50), guide = guide_axis(n.dodge=2)) + 
  theme_bw() + 
  labs(x = "Total Number of Establishments", y = "5 Digit NAICS Label", fill = "3-Digit NAICS Label", title = "Small-Medium Establishments (20-49 employees)", subtitle = title_fun(cbp_2019_clean, 241)) + 
  theme(legend.position = "bottom")
...
```{r, fig.width=11,fig.height=8}
small_med_map <- estab_size_circles(manf_share_clean, cbp_2019_clean, 241, EMPSZES_LABEL, 4375, .9, color_opac = 0.5, border_size = 0.1)
...
```{r, fig.width=11,fig.height=8}
small_med_map_overlap <- estab_size_circles(med_small_overlap_full, cbp_2019_clean, 241, EMPSZES_LABEL, 4375, .9, color_opac = 0.5, border_size = 0.1)
...
```{r}
small_col <- get_col_match(cbp_2019_clean, 230)

small_graph <- cbp_2019_clean %>% 
  ...
  scale_y_discrete(labels = scales::label_wrap(50), guide = guide_axis(n.dodge=2)) + 
  theme_bw() + 
  labs(x = "Total Number of Establishments", y = "5 Digit NAICS Label", fill = "3-Digit NAICS Label", title = "Small Establishments (10-19 employees)", subtitle = title_fun(cbp_2019_clean, 230)) + 
  theme(legend.position = "bottom")
...
```{r, fig.width=11,fig.height=8}
small_map <- estab_size_circles(manf_share_clean, cbp_2019_clean, 230, EMPSZES_LABEL, 2000, .9, color_opac = 0.5, border_size = 0.1)
...
```{r, fig.width=11,fig.height=8}
small_map_overlap <- estab_size_circles(small_med_map_overlap, cbp_2019_clean, 230, EMPSZES_LABEL, 2000, .7, color_opac = 0.5, border_size = 0.1)
...
```{r, fig.width=11,fig.height=8}
v_small_col <- get_col_match(cbp_2019_clean, 220)

v_small_graph <- cbp_2019_clean %>% 
  ...
  scale_y_discrete(labels = scales::label_wrap(50), guide = guide_axis(n.dodge=2)) + 
  theme_bw() + 
  labs(x = "Total Number of Establishments", y = "5 Digit NAICS Label", fill = "3-Digit NAICS Label", title = "Very Small Establishments (5-9 employees)", subtitle = title_fun(cbp_2019_clean, 220)) + 
  theme(legend.position = "bottom")
...
```{r, fig.width=11,fig.height=8}
v_small_map <- estab_size_circles(manf_share_clean, cbp_2019_clean, 220, EMPSZES_LABEL, 1000, .7, color_opac = 0.5, border_size = 0.1)

v_small_map
```

```{r, fig.width=11,fig.height=8}
v_small_map_overlap <- estab_size_circles(small_map_overlap, cbp_2019_clean, 220, EMPSZES_LABEL, 1000, .7, color_opac = 0.5, border_size = 0.1)
...
At this point, it is worth bringing in mining data, to control for natural resources.

```{r}
non_manf_data <- readRDS(here("County Data/non_manf_2019.RDS")) %>% 
  filter(NAICS2017 == 21) %>% 
  rename(county_fips = county, state_fips = state) %>% 
  mutate(naics_3digit_label = "Mining, quarrying, and oil and gas extraction") %>% 
  mutate(emp_tot = EMP)
```

```{r}
estab_size_circles_dynamic(v_small_map_overlap, non_manf_data, 1, EMPSZES_LABEL, 50000, fill_opac = 1, color_opac = 0)
...
```{r, fig.width=11,fig.height=8}
tiny_col <- get_col_match(cbp_2019_clean, 210)

tiny_graph <- cbp_2019_clean %>% 
  ...
  scale_y_discrete(labels = scales::label_wrap(50), guide = guide_axis(n.dodge=2)) + 
  theme_bw() + 
  labs(x = "Total Number of Establishments", y = "5 Digit NAICS Label", fill = "3-Digit NAICS Label", title = "Tiny Establishments (Under employees)", subtitle = title_fun(cbp_2019_clean, 210)) + 
  theme(legend.position = "bottom")
...
```{r, fig.width=11,fig.height=8}
tiny_map <- estab_size_circles(manf_share_clean, cbp_2019_clean, 210, EMPSZES_LABEL, 500, .6, color_opac = 0.3, border_size = 0.1)
tiny_map
```
...
```{r, fig.width=11,fig.height=8}
tiny_map_overlap <- estab_size_circles(v_small_map_overlap, cbp_2019, 210, EMPSZES_LABEL, 500, .6, color_opac = 0, border_size = 0.1)
tiny_map_overlap
```
