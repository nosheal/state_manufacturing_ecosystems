...
```{r, echo = FALSE}
emp_2019 <- build_map(state_manf_2019, emp, emp_thousands, "BuPu", "Avg Monthly Emploment", " (NAICS 31-33)", "2019 NAICS 31-33 <br/> Monthly Employees")
...
gdp_emp_2019 <- state_manf_2019 %>% 
  ...
  geom_text_repel(aes(x = gdp_manf, y = emp, label = STUSPS), max.overlaps = 30) + 
  labs(x = "NAICS 31-33 Annual GDP in Billions of $2012", y = "NAICS 31-33 Average Monthly Employees, Thousands", title = "Manufacturing GDP and Employment Tightly Correlated") + 
  guides(size = "none") + 
  theme_bw()
...
```{r}
state_categorization_2019 %>% 
  saveRDS(here("State Data/state_quadrants_1.RDS"))
```
...
county_emp_graph <- county_emp_calcs %>% 
  ...
  scale_fill_distiller(palette = "Spectral", labels = scales::label_percent()) + 
  labs(x = "County manufacturing (31-33) employment share of state employment", y = "", fill = "County Share of State Employment") + 
  scale_x_continuous(labels = scales::percent) + 
  theme_bw() + 
  axis_theme
...
concentration_comp <- county_ind_comp %>% 
  left_join(state_categorization_2019 %>% select(state_abbr, state_quadrant)) %>%
  ggplot() + 
  geom_point(aes(x = ind_hhi, y = county_hhi, fill = state_quadrant), shape = 21, size = 3) + 
  geom_text_repel((aes(x = ind_hhi, y = county_hhi, label = state_abbr)), size = 3) + 
  labs(x = "3-digit Industry Employment HHI by State", y = "2-digit County Employment HHI by State", fill = "2019 State Manufacturing Share Benchmark") + 
  scale_fill_brewer(palette = "Set1") + 
  guides(fill = "none") + 
  theme_bw() + 
  axis_theme
...
ind_size_all <- size_all_clean %>% 
  ...
  scale_fill_brewer(palette = "Paired") + 
  labs(x = "Log Establishments in State, by Industry", y = "", fill = "") + 
...
```{r}
size_manf <- size_all_clean %>% 
  filter(year == 2019, industry_code == "31-33") %>% 
  left_join(area_codes) %>% 
  left_join(state_categorization_2019 %>% select(area_fips, year, state_gdp, state_quadrant)) %>% 
  filter(!is.na(state_abbr)) %>% 
  ggplot() + 
  geom_col(aes(x = log(estabs), y = reorder(state_abbr, state_gdp), fill = reorder(size_title, size_code)), color = "black") + 
  scale_fill_brewer(palette = "Paired") + 
  labs(x = "Log Manufacturing Establishments in State, by Size", y = "", fill = "", title = "Not all states have very large manufacturing establishments") + 
  guides(fill = guide_legend(reverse = TRUE)) + 
  theme_bw() + 
  axis_theme 
...
# Industrial Composition Trends 

Because of our above findings (or lack-thereof) about industrial concentration and growth, we now explore sub-sector industrial composition trends within the manufacturing sector. 
...
Over time, certain sectors see more volatilty than others. 


```{r}
ind_time_gdp <- state_ind %>% 
  mutate(Description = str_remove(Description, "manufacturing")) %>% 
  rename(area_title = GeoName) %>% 
  left_join(area_codes) %>% 
  filter(!is.na(state_abbr)) %>% 
  left_join(manf_data %>% select(state_abbr, year, gdp_manf)) %>% 
  group_by(state_abbr, year) %>%  
  mutate(manf_gdp = sum(manf_ind_gdp), 
         ind_manf_perc = manf_ind_gdp / manf_gdp) 
```

```{r}
ind_time_gdp_na <- ind_time_gdp %>% 
  filter(year < 2020 & year > 2010) %>% 
  mutate(manf_ind_gdp = case_when(
    manf_ind_gdp == 0 ~ NA_real_, 
    manf_ind_gdp != 0 ~ manf_ind_gdp
  ))
```


```{r}
subsector_gdp <- ind_time_gdp_na %>% 
  group_by(Description, year) %>%
  reframe(subsector_gdp = sum(manf_ind_gdp, na.rm = TRUE))
```


```{r}
ind_var <- subsector_gdp %>% 
  group_by(Description) %>% 
  reframe(subsector_gdp_var = sd(subsector_gdp, na.rm = TRUE)) %>% 
  left_join(subsector_gdp %>% 
              group_by(Description) %>% 
              reframe(subsector_gdp_mean = mean(subsector_gdp, na.rm = TRUE))) %>% 
  ungroup() %>% 
  mutate(mean_var = mean(subsector_gdp_var, na.rm = TRUE), 
         var_index_sector = subsector_gdp_var / mean_var, 
         var_perc_sector = subsector_gdp_var / subsector_gdp_mean)
```

```{r}
gdp_ind_vector <- c(brewer.pal(9, "Greys")[4], "#BEAED4", "#FDC086", "#FDBF6F",  "#386CB0", "#FB8072", brewer.pal(9, "Greys")[3], "#80B1D3", "#F0027F", "#B15928",  "#4DAF4A", "#A6761D", "#F1E2CC", "#6A3D9A", "#E78AC3", "#CBD5E8", "#666666", brewer.pal(9, "Greys")[5],  brewer.pal(9, "Greys")[6], brewer.pal(9, "Greys")[7]) 
```

```{r}
subsector_gdp_var <- subsector_gdp %>% 
  ggplot(aes(x = subsector_gdp, y = Description, fill = Description)) + 
  geom_density_ridges() + 
  scale_fill_manual(values = gdp_ind_vector) + 
  guides(fill = "none") + 
  theme_bw() + 
  labs(x = "3-Digit Subsector GDP, 2010-2019", y = "3 Digit SuManufacturing Subsector") + 
  axis_theme
...
my_grid <- my_grid %>% filter(code != "DC")
```

```{r}
ind_time_gdp_cat <- ind_time_gdp %>% 
  left_join(
    state_categorization_2019 %>% select(state_abbr, state_quadrant, manf_gdp_share, manf_emp_share)
  )
```

Within the 3-digit share of manufacturing GDP, we see that certain states have much larger manufacturing sectors than others, and that the scale and composition of the manufacturing sector changes heterogenously over time. Our challenge is to develop meaningful descriptions of this heterogeneity. 


```{r, eval = FALSE}
 [1] "#7FC97F" "#BEAED4" "#FDC086" "#FFFF99" "#386CB0" "#F0027F" "#BF5B17" "#666666" "#1B9E77" "#D95F02"
[11] "#7570B3" "#E7298A" "#66A61E" "#E6AB02" "#A6761D" "#666666" "#A6CEE3" "#1F78B4" "#B2DF8A" "#33A02C"
[21] "#FB9A99" "#E31A1C" "#FDBF6F" "#FF7F00" "#CAB2D6" "#6A3D9A" "#FFFF99" "#B15928" "#FBB4AE" "#B3CDE3"
[31] "#CCEBC5" "#DECBE4" "#FED9A6" "#FFFFCC" "#E5D8BD" "#FDDAEC" "#F2F2F2" "#B3E2CD" "#FDCDAC" "#CBD5E8"
[41] "#F4CAE4" "#E6F5C9" "#FFF2AE" "#F1E2CC" "#CCCCCC" "#E41A1C" "#377EB8" "#4DAF4A" "#984EA3" "#FF7F00"
[51] "#FFFF33" "#A65628" "#F781BF" "#999999" "#66C2A5" "#FC8D62" "#8DA0CB" "#E78AC3" "#A6D854" "#FFD92F"
[61] "#E5C494" "#B3B3B3" "#8DD3C7" "#FFFFB3" "#BEBADA" "#FB8072" "#80B1D3" "#FDB462" "#B3DE69" "#FCCDE5"
[71] "#D9D9D9" "#BC80BD" "#CCEBC5" "#FFED6F"
```


```{r,  fig.width=11,fig.height=8}
ind_subsector_time <- ind_time_gdp_cat %>% 
  mutate(sub_sector_share = 100*ind_share_state) %>% 
  ggplot() + 
  geom_col(aes(x = year, y = sub_sector_share, fill = Description)) + 
  scale_fill_manual(values = gdp_ind_vector) + 
  ylim(0, 35) + 
  theme_bw() + 
  labs(x = "Year", y = "Sub-Sector (3 Digit) Share of State GDP") +
  theme(axis.text.x = element_text(angle = 90))

ind_state_geo <- ind_subsector_time + facet_geo(~ state_abbr, grid = my_grid) + guides(fill = "none")

ind_state_geo
```

```{r ,  fig.width=16,fig.height=8}
legend <- get_legend(ind_subsector_time)
ggdraw(legend)
```

In terms of share of State GDP, we see that the largest manufacturing economies appear to be located in what might be considered "middle" America.

Computer and electronic product manufacturing, as well as chemical manufacturing, petroleum product manufacturing, and transportation equipment manufacturing (motor vehicles as well as other transportation equipment, distinctly) appear to make up the largest shares of national manufacturing capabilities. The largest manufacturing sectors in terms of share of state GDP appear to be in what might considered "middle-America" states, as well as on the West coast, and in the Mountain West. 

We can also further investigate how the total yearly manufacturing share changes over time as well, to get a better understanding of regional specialties and trends. 

```{r,  fig.width=11,fig.height=8}
ind_subsector_time <- ind_time_gdp_cat %>% 
  mutate(sub_sector_share = 100*ind_manf_perc) %>% 
  ggplot() + 
  geom_col(aes(x = year, y = sub_sector_share, fill = Description)) + 
  scale_fill_manual(values = gdp_ind_vector) + 
  theme_bw() + 
  labs(x = "Year", y = "Sub-Sector (3 Digit) Share of State Manufacturing GDP") +
  theme(axis.text.x = element_text(angle = 90))

ind_share_geo <- ind_subsector_time + facet_geo(~ state_abbr, grid = my_grid)  + 
  guides(fill = "none")

ind_share_geo
```

We see what appears to be a strong computer and electronics sector in the west coast and mountain west (NM, AZ, CO, CA, OR, ID), a chemical manufacturing sector in the northeast, motor vehicle manufacturing across MI, IN, KY, TN, and AL. Food and beverage manufacturing appears more distributed across the country than other sectors. 

In addition, some states (OH, AL, MS, TN, KY) appear far more evenly distributed than others (CA, LA, MT, WY). 

From here, we might also be interested in patterns across states grouped by their relative share of manufacturing GDP/Employment against the national manufacturing share, since this categorization appeared to be useful previously. 

```{r}
state_q <- ind_time_gdp_cat %>% ungroup() %>%  select(state_quadrant) %>% distinct() %>% unlist()
```


```{r}
subsector_time <- function(data, var, tit){
  data %>% 
  ggplot() + 
  geom_col(aes(x = year, y = {{ var }}, fill = Description)) + 
  scale_fill_manual(values = gdp_ind_vector) + 
  facet_wrap(~reorder(state_abbr, manf_gdp_share)) +
  theme_bw() + 
  labs(x = "Year", y = "Sub-Sector (3 Digit) Share of State GDP", title = tit) +
  theme(axis.text.x = element_text(angle = 90)) + 
  guides(fill = "none")
}
```
...
```{r}
q_1 <- ind_time_gdp_cat %>% 
  filter(state_quadrant == state_q[1]) %>% 
  mutate(sub_sector_share = 100*ind_share_state) %>% 
  subsector_time(sub_sector_share, state_q[1]) + 
  labs(y = "")

q_2 <- ind_time_gdp_cat %>% 
  filter(state_quadrant == state_q[2]) %>% 
  mutate(sub_sector_share = 100*ind_share_state) %>% 
  subsector_time(sub_sector_share, state_q[2]) + 
  labs(y = "")

q_3 <- ind_time_gdp_cat %>% 
  filter(state_quadrant == state_q[3]) %>% 
  mutate(sub_sector_share = 100*ind_share_state) %>% 
  subsector_time(sub_sector_share, state_q[3]) 

q_4 <- ind_time_gdp_cat %>% 
  filter(state_quadrant == state_q[4]) %>% 
  mutate(sub_sector_share = 100*ind_share_state) %>% 
  subsector_time(sub_sector_share, state_q[4])
```

```{r, fig.width=11,fig.height=8}
by_quad_gdp_1 <- (q_3+ q_1) / (q_2 + q_4)

by_quad_gdp_1
```

At this point, we do not see any glaringly obvious patterns, but this ordering is still useful for us to examine. We can also focus more specifically on share. 

```{r}
subsector_time_share <- function(data, var, tit){
  data %>% 
  ggplot() + 
  geom_col(aes(x = year, y = {{ var }}, fill = Description)) + 
  scale_fill_manual(values = gdp_ind_vector) + 
  facet_wrap(~reorder(state_abbr, manf_gdp_share)) +
  theme_bw() + 
  labs(x = "Year", y = "Sub-Sector (3 Digit) Share of State Manufacturing GDP", title = tit) +
  theme(axis.text.x = element_text(angle = 90)) + 
  guides(fill = "none")
}
```
...
```{r}
q_1_gdp_share <- ind_time_gdp_cat %>% 
  filter(state_quadrant == state_q[1]) %>% 
  mutate(sub_sector_share = 100*ind_manf_perc) %>% 
  subsector_time_share(sub_sector_share, state_q[1]) + 
  labs(y = "")

q_2_gdp_share <- ind_time_gdp_cat %>% 
  filter(state_quadrant == state_q[2]) %>% 
  mutate(sub_sector_share = 100*ind_manf_perc) %>% 
  subsector_time_share(sub_sector_share, state_q[2]) + 
  labs(y = "")

q_3_gdp_share <- ind_time_gdp_cat %>% 
  filter(state_quadrant == state_q[3]) %>% 
  mutate(sub_sector_share = 100*ind_manf_perc) %>% 
  subsector_time_share(sub_sector_share, state_q[3]) 

q_4_gdp_share <- ind_time_gdp_cat %>% 
  filter(state_quadrant == state_q[4]) %>% 
  mutate(sub_sector_share = 100*ind_manf_perc) %>% 
  subsector_time_share(sub_sector_share, state_q[4])
```

```{r,  fig.width=11,fig.height=8}
by_quad_gdp_2 <- (q_3_gdp_share+ q_1_gdp_share) / (q_2_gdp_share + q_4_gdp_share)

by_quad_gdp_2
```


We see that the industrial composition of states changes heterogeneously, and for some states - dramatically, over time. In addition, it appears that certain manufacturing sectors (such as petroleum, food and beverage, and computer and electronic products). As such, we might need to create a sub-sector level measure of "stability", and then examine state-level variance in across sub-sectors that controls for the sub-sector level measure of "stability." 

However, before we do this, we want to also perform the above analysis for employment. 
...
```{r}
qcew_emp_simple <- qcew_emp %>% 
  select(area_fips, tot_emp = emp, year)
```
...
```{r}
state_emp_cat <- state_ind_emp %>% 
  filter(area_fips != "US000") %>% 
  left_join(qcew_emp_simple) %>%
  group_by(area_fips, year) %>% 
  mutate(manf_emp_state = sum(emp), 
         manf_state_share = emp/tot_emp, 
         ind_emp_share = emp / manf_emp_state) %>% 
  left_join(state_quads %>% ungroup() %>% 
              select(state_quadrant, area_fips, manf_emp = emp, state_abbr, manf_emp_share)) %>% 
  mutate(industry_desc = substring(industry_title, 11))
```

```{r}
state_emp_cat %>% ungroup() %>% select(industry_desc) %>% distinct() %>% arrange(industry_desc)
```
...
```{r, eval = FALSE}
n <- 21
qual_col_pals_emp = brewer.pal.info[brewer.pal.info$category == 'qual',]
col_vector_emp = unlist(mapply(brewer.pal, qual_col_pals_emp$maxcolors, rownames(qual_col_pals_emp)))
```

Note: will need to redo this with either NETS or CBP data due to employment data suppression. 


```{r,  fig.width=11,fig.height=8}
emp_subsector_time <- state_emp_cat %>% 
  mutate(sub_sector_share = 100*manf_state_share) %>% 
  filter(!is.na(state_abbr)) %>% 
  ggplot() + 
  geom_col(aes(x = year, y = sub_sector_share, fill = industry_desc)) + 
  scale_fill_manual(values = emp_ind_vector) + 
  ylim(0, 35) + 
  theme_bw() + 
  labs(x = "Year", y = "Sub-Sector (3 Digit) Share of State Avg. Annual Employment", fill = "3-Digit Manufacturing Subsector") +
  theme(axis.text.x = element_text(angle = 90)) 

emp_state_geo <- emp_subsector_time + facet_geo(~state_abbr, grid = my_grid) + 
  guides(fill = "none")
...
```{r}
input_categorizations <- readRDS(here("Input Output Data/cluster_assignments.RDS")) %>% 
  mutate(naics_3digit = naics_sub(ind_code,3))
```
...
```{r ,  fig.width=11,fig.height=8}
legend <- get_legend(emp_subsector_time)
ggdraw(legend)
```


We see that state composition of manufacturing employment is very different than state composition of manufacturing GDP. 


```{r,  fig.width=11,fig.height=8}
emp_subsector_share <- state_emp_cat %>% 
  mutate(sub_sector_share = 100*ind_emp_share) %>% 
  filter(!is.na(state_abbr)) %>% 
  ggplot() + 
  geom_col(aes(x = year, y = sub_sector_share, fill = industry_desc)) +
  scale_fill_manual(values = emp_ind_vector) + 
  theme_bw() + 
  labs(x = "Year", y = "Sub-Sector (3 Digit) Share of Manufacturing Avg. Annual Employment") +
  theme(axis.text.x = element_text(angle = 90)) + 
  guides(fill = "none")

emp_share_geo <- emp_subsector_share + facet_geo(~ state_abbr, grid = my_grid) 

emp_share_geo
```

In general, manufacturing employment is far more evenly distributed tha manufacturing GDP across sub-sectors. 


```{r}
subsector_time_emp <- function(data, var, tit){
  data %>% 
  ggplot() + 
  geom_col(aes(x = year, y = {{ var }}, fill = industry_desc)) + 
  scale_fill_manual(values = emp_ind_vector) + 
  facet_wrap(~reorder(state_abbr, manf_emp_share)) +
  theme_bw() + 
  labs(x = "Year", y = "Sub-Sector (3 Digit) Share of \nState Avg. Annual Employment", title = tit) +
  theme(axis.text.x = element_text(angle = 90)) + 
  guides(fill = "none")
}
```
...
```{r}
q_1_emp <- state_emp_cat %>% 
  filter(state_quadrant == state_q[1]) %>% 
  mutate(sub_sector_share = 100*manf_state_share) %>% 
  subsector_time_emp(sub_sector_share, state_q[1]) + 
  labs(y = "")

q_2_emp <- state_emp_cat %>% 
  filter(state_quadrant == state_q[2]) %>% 
  mutate(sub_sector_share = 100*manf_state_share) %>% 
  subsector_time_emp(sub_sector_share, state_q[2]) + 
  labs(y = "")

q_3_emp <- state_emp_cat %>% 
  filter(state_quadrant == state_q[3]) %>% 
  mutate(sub_sector_share = 100*manf_state_share) %>% 
  subsector_time_emp(sub_sector_share, state_q[3]) 

q_4_emp <- state_emp_cat %>% 
  filter(state_quadrant == state_q[4]) %>% 
  mutate(sub_sector_share = 100*manf_state_share) %>% 
  subsector_time_emp(sub_sector_share, state_q[4])
```

```{r,  fig.width=11,fig.height=8}
by_quad_emp <- (q_3_emp + q_1_emp) / (q_2_emp + q_4_emp)
...
```{r}
subsector_time_emp_share <- function(data, var, tit){
  data %>% 
  ggplot() + 
  geom_col(aes(x = year, y = {{ var }}, fill = industry_desc)) + 
  scale_fill_manual(values = emp_ind_vector) + 
  facet_wrap(~reorder(state_abbr, manf_emp_share)) +
  theme_bw() + 
  labs(x = "Year", y = "Sub-Sector (3 Digit) Share of \nState Manufacturing Avg. Annual Employment", title = tit) +
  theme(axis.text.x = element_text(angle = 90)) + 
  guides(fill = "none")
}
```
...
```{r}
q_1_emp_share <- state_emp_cat %>% 
  filter(state_quadrant == state_q[1]) %>% 
  mutate(sub_sector_share = 100*ind_emp_share) %>% 
  subsector_time_emp_share(sub_sector_share, state_q[1]) + 
  labs(y = "")

q_2_emp_share <- state_emp_cat %>% 
  filter(state_quadrant == state_q[2]) %>% 
  mutate(sub_sector_share = 100*ind_emp_share) %>% 
  subsector_time_emp_share(sub_sector_share, state_q[2]) + 
  labs(y = "")

q_3_emp_share <- state_emp_cat %>% 
  filter(state_quadrant == state_q[3]) %>% 
  mutate(sub_sector_share = 100*ind_emp_share) %>% 
  subsector_time_emp_share(sub_sector_share, state_q[3]) 

q_4_emp_share <- state_emp_cat %>% 
  filter(state_quadrant == state_q[4]) %>% 
  mutate(sub_sector_share = 100*ind_emp_share) %>% 
  subsector_time_emp_share(sub_sector_share, state_q[4])
```

```{r,  fig.width=11,fig.height=8}
by_quad_emp_share <- (q_3_emp_share + q_1_emp_share) / (q_2_emp_share+ q_4_emp_share)
...
Indeed, we confirm that some manufacturing sub-sectors have a much greater standard deviation across years than others. For ease of comparison, we benchmark sub-sector variation against the mean of variation to create an index. Based on a preliminary analysis, we see that for some states and sub-sectors (e.g Paper manufacturing in Montana), there is a substantial drop between 2010 and 2011. To account for these outliers, we might want to drop the year 2010 from our data. 
...
```{r}
subsector_gdp_var <- subsector_gdp %>% 
  ggplot(aes(x = subsector_gdp, y = reorder(Description, subsector_gdp), fill = Description)) + 
  geom_density_ridges() + 
  scale_fill_manual(values = gdp_ind_vector) + 
  guides(fill = "none") + 
  theme_bw() + 
...
ind_var %>% 
  ggplot() + 
  geom_point(aes(x = subsector_gdp_mean, y = reorder(Description, subsector_gdp_mean), fill = Description), alpha = 0.7, color = "black", shape = 21, size = 4) + 
  geom_segment(aes(x = subsector_gdp_mean - subsector_gdp_var, xend = subsector_gdp_mean + subsector_gdp_var, y = reorder(Description, subsector_gdp_mean), yend = reorder(Description, subsector_gdp_mean), color = Description), size = 3) + 
  labs(x = "Mean Value of Manufacturing 3-Digit Subsector, and Standard Deviation across years", y = "Manufacturing Sub-Sector GDP (3-Digit)") + 
...
**Some sub-sectors have higher "volatility" accross years than others**

```{r}
ind_var_index <- ind_var %>% 
  ggplot() + 
  geom_col(aes(x = var_index_sector, y = reorder(Description, var_index_sector), fill = Description), alpha = 0.7) + 
  labs(x = "Standard Deviation in Sub-Sector GDP as \n 
       Index of Mean SD across sub-sectors (Mean = 7751.957) \n2010-2019", y = "Manufacturing Sub-Sector GDP (3-Digit)") + 
  scale_fill_manual(values = gdp_ind_vector) + 
  theme_bw() + 
  guides(fill = "none") + 
  axis_theme
...
```{r}
ind_var_perc <- ind_var %>% 
  ggplot() + 
  geom_col(aes(x = var_perc_sector, y = reorder(Description, var_perc_sector)), alpha = 0.7, color = "black") + 
  scale_x_continuous(labels = scales::percent) + 
  labs(x = "Standard Deviation in Sub-Sector GDP as \n 
       % of Average Sub-Sector GDP \n2010-2019", y = "Manufacturing Sub-Sector GDP (3-Digit)") + 
  theme_bw() + 
  axis_theme
...
We can then benchmark state-level variation in sub-sector GDP against sub-sector level variation to understand which states have higher than expected variance in sub-sector GDP than would be otherwise expected. 

```{r}
subsector_state_benchmarks <- ind_time_gdp_na %>% 
  filter(year < 2020 & year > 2010) %>% 
  group_by(Description, state_abbr) %>%
  reframe(subsector_gdp_var_state = sd(manf_ind_gdp, na.rm = TRUE)) %>% 
  left_join(ind_time_gdp_na %>% 
              filter(year < 2020 & year > 2010) %>% 
              group_by(Description, state_abbr) %>% 
              reframe(subsector_gdp_mean_state = mean(manf_ind_gdp, na.rm = TRUE))) %>% 
  group_by(Description) %>% 
  mutate(mean_index = mean(subsector_gdp_var_state, na.rm = TRUE), 
         var_index_state = subsector_gdp_var_state / mean_index,
    var_perc_state = subsector_gdp_var_state / subsector_gdp_mean_state) %>% 
  left_join(ind_var %>% 
              select(Description, var_index_sector, var_perc_sector)) %>% 
  mutate(var_index_state / var_index_sector) 
```


```{r, eval = FALSE}
subsector_state_benchmarks %>% 
  ggplot() + 
  geom_text(aes(x = log(subsector_gdp_mean_state), y = var_index_state, label = state_abbr)) + 
  geom_hline(aes(yintercept = var_index_sector)) + 
...
subsector_state_benchmarks %>% 
  ggplot() + 
  geom_text(aes(x = log(subsector_gdp_mean_state), y = var_perc_state, label = state_abbr)) + 
  geom_hline(aes(yintercept = var_perc_sector)) + 
  facet_wrap(~Description) + 
  theme_bw()
```
...
ind_time_gdp %>% 
  ungroup() %>% 
  group_by(state_abbr, Description) %>% 
  mutate(last_year_change = lag(ind_manf_perc)) %>% 
  ggplot() + 
  geom_point(aes(last_year_change, ind_manf_perc, fill = Description), color = "black", shape = 21, size = 3, alpha = 0.7) + 
  scale_fill_manual(values = col_vector) + 
  theme_bw()
```
...
At the sub-sector level, we expect a certain level of variation in sub-sector level manufacturing GDP: E(var(GDP_i)). 

At the state-level, we expect a certain level of variation in state level manufacturing GDP: E(var(GDP_s)) 

Controlling for the expected state-level manufacturing GDP variation, and the expected sub-sector level manufacturing GDP variation, what is the unexplained state, sub-sector level variation that is unexplained and might be due to policy? 
Thus, we estimate: $$\hat{gdpvar_{s,i}} = \beta_0 + \beta_1*gdpvar_i + \beta_2*gdpvar_s + \epsilon_{s,i}$$ 


```{r}
state_var <- ind_time_gdp_na %>% 
  select(state_abbr, year, manf_gdp, state_gdp) %>% 
  distinct() %>% 
  mutate(manf_share = manf_gdp/state_gdp) %>% 
  group_by(state_abbr) %>% 
  reframe(manf_var_state = sd(manf_share))
```


```{r}
var_test <- ind_time_gdp_na %>% 
  group_by(state_abbr, Description) %>% 
  reframe(state_ind_var = sd(manf_ind_gdp, na.rm = TRUE)) %>% 
  left_join(state_var) %>% 
  left_join(ind_var)
```
...
```{r}
var_model_1 <- lm(state_ind_var ~ subsector_gdp_var, data = var_test)
```

```{r}
var_model_2 <- lm(state_ind_var ~ subsector_gdp_var + manf_var_state, data = var_test)
```

```{r}
var_model_3 <- lm(state_ind_var ~ 0 + Description, data = var_test)
```
...
```{r}
export_summs(var_model_1, var_model_2)
```

Here, we see that the variation in subsector-GDP might explain a small percent of the variation in state-subsector-level trends. Adding state-level controls does not improve the model substantially, and is not a significant correlate with state-sub-sector level trends. 

Additionally, we can confirm that certain subsectors are inherently more responsible for year-to-year variation than others by estimating a sub-sector fixed effects model. 

```{r}
export_summs(var_model_3)
```

A subset of the main sectors that we identified earlier as high-volatility sub-sectors are statistically correlated with higher year-to-year variation n state-subsector level trends: (Chemical, Computer & Electronics, Food & Beverage, Machinery, Transportation, Motor Vehicles, Petroleum)

```{r}
significant_list <- c("Chemical", "Computer and electronic", "Food and beverage", "Machinery", "Other transporation", "Petroleum", "Motor vehicles")
```

```{r}
ind_var_sig <- ind_var %>% 
             mutate(significant = case_when(
               str_detect(Description, paste(significant_list, collapse = "|")) ~ 1))
```

```{r}
ind_var_perc + 
  geom_col(data = ind_var_sig, aes(x = var_perc_sector, y = reorder(Description, var_perc_sector), fill = significant), alpha = 0.7, color = "black") + 
  guides(fill = "none")
```

We then add the residuals and the fitted values back into our dataframe. 

```{r}
var_test$residuals_1 <- var_model_1$residuals
var_test$fitted_1 <- var_model_1$fitted.values
```

We see that the distribution of residuals is highly skewed. 

```{r}
hist(var_model_1$residuals, xlab = "Residual", main = "Histogram of residuals \nwhen controlling subsector-level variation")
```


```{r}
var_test <-  var_test  %>% 
  mutate(log_residuals = log(abs(residuals_1)))
```

However, when taking the log of the absolute value of the residuals, the results appear to be more normally distributed, centered around 4. 

```{r}
hist(var_test$log_residuals, xlab = "Log Absolute Value Residual", main = "Histogram of log absolute value residuals \nwhen controlling subsector-level variation")
```


Based on where in the distribution of log absolute value of the residual a specific state-subsector combination lies, we can further group state-subsectors into discrete categories

State-Subsector Variation as Expected (< 1); State-Subsector Variation Slightly Higher than Expected (1-4); State-Subsector Variation Higher than Expected (4-6); State-Subsector Variation Much Higher than Expected (6+). 

```{r}
var_test_cats <- var_test %>% 
  mutate(residuals_cat = case_when(
    log_residuals <= 1 ~ "As Expected", 
    log_residuals > 1 & log_residuals < 4 ~ "Slightly Higher than Expected", 
    log_residuals >= 4  & log_residuals < 6 ~ "Higher than Expected", 
    log_residuals >= 6 ~ "Much Higher than Expected"
  ))
```

As expected from the above graph, most of our observations fall in the "Higher than Expected" category. 

```{r}
var_test_cats %>% 
  group_by(residuals_cat) %>% 
  count()
```
...
var_test %>% 
  ggplot() + 
  geom_point(aes(x = fitted_1, y = log_residuals, fill = Description), shape = 21, color = "black", size = 3) + 
  scale_fill_manual(values = col_vector) + 
  facet_wrap(~state_abbr) + 
  theme_bw()
```

**Similarly, some states have more "volatility" in their manufacturing sectors than others**
```{r}
state_var_graph <- state_var %>% 
  ggplot() + 
  geom_col(aes(x = manf_var_state, y = reorder(state_abbr, manf_var_state)), alpha = 0.7, color = "black")+ 
  scale_x_continuous(labels = scales::percent) + 
  theme_bw() + 
  labs(x = "Standard Deviation of State Manufacturing Share of GDP \n as % of Mean State Manufacturing Share of GDP 2010-2019", y = "")
