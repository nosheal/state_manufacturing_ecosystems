...
So far, we have seen evidence that states vary by their manufacturing share and size of GDP/Employment, as well as the major sub-industries that make up their manufacturing economy.

As another way to examine industry concentration in states, we might then want to look at how the state HHI index corresponds with its "max" location quotient, which would be the industry in the state that the state has the largest share of, relative to the national average.

```{r}
state_ind_benchmark <- state_ind %>% 
  filter(year == 2019) %>%
  rename(NAME = GeoName) %>% 
  left_join(state_manf %>% 
              filter(year == 2019) %>% 
              select(NAME, manf_gdp_share, manf_emp_share, state_gdp, gdp_manf))
```
...
We again see evidence that some states are more concentrated than others. Once again, our challenge now lies in combining these descriptive statistics about states to start categorizing states individually. Before proceeding to that task, we explore a few more basic statistics about state manufacturing economies.
...
We can also then examine the corrleation between percentage change in NAICS 31-33 employment and percentage change in GDP. We see that these measures are roughly correlated. 
...
We now want to benchmark changes in the manufacturing sector against changes in the state economy as a whole. 
