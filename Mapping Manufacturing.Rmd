...
concentration_comp <- county_ind_comp %>% 
  ...
  labs(x = "Industry HHI by State", y = "County HHI by State", title = "Apparent weak inverse correlation between county concentration and industry concentration?", fill = "2019 State Manufacturing Share Benchmark") + 
  scale_fill_brewer(palette = "Set1") + 
  theme_bw() + 
  axis_theme
...
```{r,  fig.width=11,fig.height=8}
ind_subsector_time <- ind_time_gdp %>% 
  mutate(sub_sector_share = 100*ind_manf_perc) %>% 
  ggplot() + 
  geom_col(aes(x = year, y = sub_sector_share, fill = Description), color = "black") + 
...
ind_time_geo <- ind_subsector_time + facet_geo(~ state_abbr) 
...
```{r}
ind_time_geo <- ind_subsector_time + facet_geo(~ state_abbr) 
ind_time_states <- ind_subsector_time + facet_wrap(~state_abbr)
```

```{r}
ggplotly(ind_time_states)
```
...
ind_state_geo <- ind_subsector_time + facet_geo(~ state_abbr) 
...
ind_var_index <- ind_var %>% 
  ggplot() + 
  geom_col(aes(x = var_index_sector, y = reorder(Description, var_index_sector)), alpha = 0.7, color = "black") + 
