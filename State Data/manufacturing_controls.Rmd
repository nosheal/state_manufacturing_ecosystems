...
```{r}
saveRDS(gdp_sum, here("State Data/national_gdp.RDS"))
```
...
```{r}
# New facet label names for manf_sign variable
manf_sign.labs <- c("FALSE", "TRUE")
names(manf_sign.labs) <- c("Manf GDP \n% Change < All Industry GDP \n% Change", "Manf GDP \n% Change > All Industry GDP \n% Change")
```
...
emp_2010_2019_b <- manf_data %>% 
  ...
  guides(fill = "none", color = "none") + 
  coord_flip() + 
  facet_wrap(~manf_sign, labeller = labeller(manf_sign = manf_sign.labs)) + 
  labs(y = "Manufacturing Share of State Employment \n(2010-2019)", x = "") + 
  theme_bw() + 
  axis_theme 
...
```{r}
manf_clean <- manf_data %>% 
  left_join(state_gdp %>% 
              select(year, state_gdp, area_fips)) 
```

```{r}
saveRDS(manf_clean, here("State Data/manf_clean.RDS"))
```
...
```{r}
saveRDS(ind_conc, here("State Data/industry_emp.RDS"))
```
