...
```{r}
rd_out_graph1 <- rd_inputs %>% 
  group_by(naics_3digit_label) %>% 
  mutate(max_rd = max(rd_ind_perc), 
         ind_label = case_when(rd_ind_perc > 0.05/100 ~ industry_desc)) %>% 
...
```{r}
cbp_io %>% saveRDS(here("Input Output Data/cbp_manf_IO_crosswalk.RDS"))
```


```{r}
#Final Mapping function (dynamic scaling with number of employees)
estab_size_circles_inputs <- function(map, data, condition, var, rad, fill_opac, color_opac = 1, border_size = 2){
  ...
  col_fun <- colorFactor(h_cols, map_data$h_clusters)
...
```{r}
cbp_input_ref <- cbp_2019 %>% left_join(cbp_io) %>% left_join(cluster_assignments)
```


```{r}
input_manf <- estab_size_circles_inputs(gdp_manf_share, cbp_input_ref, 1, EMPSZES_LABEL, rad = 50000, fill_opac = .8, color_opac = 0.3)
```
...
```{r}
input_manf <- estab_size_circles_inputs(gdp_manf_share, cbp_2019, 1, EMPSZES_LABEL, rad = 50000, fill_opac = .8, color_opac = 0.3)
```
...
ind_geo_graph0 <- state_diversity_geog %>%
  ...
  left_join(state_quads) %>%
  ggplot() + 
  geom_point(aes(x = ind_shannon_emp, state_shannon_emp, fill = state_quadrant), shape = 21) + 
  geom_text_repel(aes(x = ind_shannon_emp, state_shannon_emp, label = state_abbr), size = 5) + 
  scale_fill_brewer(palette = "Set1") + 
  guides(fill = "none") + 
  theme_bw() + 
  labs(x = "State Industry Shannon-Weiner Diversity Index", y = "State Geography Shannon-Weiner Diversity Index")
```
...
ind_geo_graph1 <- state_diversity_geog %>%
  ...
  ggplot() + 
  geom_point(aes(x = ind_shannon_emp, state_shannon_emp, color = state_quadrant)) + 
  scale_color_brewer(palette = "Set1")
```
