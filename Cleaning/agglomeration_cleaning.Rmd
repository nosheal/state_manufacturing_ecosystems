...
inputs_used <- which(com_list_outputs %in% input_list)
```
...
I apply a cutoff of 40%, and filter for industries that can account for at least 40% of their value of outputs produced that comes from the semiconductor and related devices manufacturing sector. 
...
supply_matrix <- output_matrix %>% rbind(ind_inputs %>% t())
```
...
Industries vary in the degree to which they supply the mixture of manufactured inputs that are used by the focal industry. We then manually correct for industries that are deemed to not "substantially supply" the target industries. These industries are defined as industries that on average, contribute less than 1% to the total supply of the vector of manufactured commodities used by the focal industry. 

```{r}
ind_use <- data.frame(industry_code = ind_list, supply_corr[233,-233], com_tot = rowSums(output_matrix) %>% round(5)) %>% 
  filter(com_tot > .01) %>% 
  left_join(ind_crosswalk) 

ind_use_col <- get_col_match(ind_use)
```


```{r}
ind_use %>% 
  ggplot(aes(x = com_tot, y = naics_3digit_label, fill = naics_3digit_label)) +
  geom_density_ridges() + 
  geom_point(shape = 21) + 
  guides(fill = "none") + 
  scale_fill_manual(values = ind_use_col) + 
  labs(x = "Total Supply of Commodities used by Focal Industry", y = "") + 
  theme_bw() + 
  axis_theme
```

For these industries, we assign a value of 0 to our matrix of correlations. 

```{r}
#Get nonsupply industries
non_cor_ind <- which(rowSums(output_matrix) < .01)
```

```{r}
#Fix values
supply_corr[, non_cor_ind] = 0 
```

```{r}
#Final DF
supply_df <- data.frame(industry_code = ind_list, supply_corr_var = supply_corr[233,-233], com_tot = rowSums(output_matrix) %>% round(5)) %>% 
  left_join(ind_crosswalk)
```

We see that we have some values that have a negative correlation. A manual inspection of these industries suggests that these industries share singular commodities with the focal industry (e.g. Scientific R&D), but otherwise do not produce relevant commodities. In most cases, industries with negative correlations are not very "related" to the focal industry (e.g. Abrasive.product.manufacturing, Soap.and.cleaning.compound.manufacturing, against Semiconductor manufacturing). As such, to correct for these cases, we take the absolute value of the correlation only in cases where the industry shares a 3-digit NAICS family. 

```{r}

#Clean DF
supply_df_clean <- supply_df %>% 
  mutate(supply_corr_var = case_when(
    supply_corr_var < 0 & naics_3digit_label == unique(input_vec_manf$naics_3digit_label) ~ -1*supply_corr_var, 
    supply_corr_var < 0 ~ 0, 
    TRUE ~ supply_corr_var
  )) 
```
...
get_industry_supply_cor <- function(industry){
  ...
  supply_results <- supply_df_clean %>% select(supply_corr_var) %>% unlist() %>% unname()
...
for (i in 1:length(ind_list)){
  ...
  mat_holder[i, ] <- get_industry_supply_cor(ind_list[i])
...
```{r}
supply_final <- results_long %>% rename(industry_code = industry_i) %>% 
  left_join(ind_crosswalk)
```


```{r, eval = FALSE}
saveRDS(supply_mats, here("Input Output Data/results_matrix_corr.RDS"))
```

```{r}
supply_mats <- readRDS(here("Input Output Data/results_matrix_corr.RDS"))
```
...
get_industry_outputs_share <- function(industry){
  ...
  #Match the commodity codes to the index of commodity codes we have (supply)
  com_list_sup <- which(com_cod %in% input_list) %>% 
    unlist() %>% 
    unname()
...
```{r, eval = FALSE}
mat_holder <- readRDS(here("Input Output Data/results_matrix.RDS"))
```
...
cbp_merge <- cbp_2019 %>% 
  ...
  mutate(county_estabs = sum(ESTAB), 
           ...
           county_perc_emp = emp_num / county_emp, 
           county_perc_estabs = ESTAB / county_estabs, 
...
```{r}
cbp_egk <- cbp_final_io %>% 
  #EGK MEASURES (S_RI)
  mutate(s_ri_estabs = county_ind_tot_estabs / county_tot_estabs, 
...
```{r}
coi_denom <- cbp_egk %>%
  select(NAME, x_r_estabs, x_r_emp) %>% 
  distinct() %>% 
  mutate(coi_denom_estabs = (1-sum(x_r_estabs**2, na.rm = TRUE)), 
         coi_denom_emp = (1-sum(x_r_emp**2, na.rm = TRUE)))
```
...
cbp_egk <- cbp_egk %>% 
  left_join(coi_denom) %>% 
  mutate(coi_num_ri_estabs = s_ri_estabs - x_r_estabs,
         coi_num_ri_emp = s_ri_emp - x_r_emp) 
```

We now create a matrix with the EGK measure of co-agglomeration. 

```{r}
egk_long <- cbp_final_io %>% 
  mutate(coi_frac_ri_estabs = coi_num_ri_estabs/coi_denom_estabs,
          coi_frac_ri_emp = coi_num_ri_emp/coi_denom_emp) %>% 
  filter(!is.na(ind_code)) %>%
  select(ind_code, NAME, coi_frac_ri_estabs, coi_frac_ri_emp, coi_num_ri_estabs, coi_num_ri_emp) 
```

```{r}
frac_ir_matrix_estabs <- egk_long %>% 
    pivot_wider(id_cols = ind_code, names_from = NAME, values_from = coi_frac_ri_estabs, values_fill = 0) %>% 
    select(-c(ind_code)) %>% 
    as.matrix()
```

```{r}
frac_ir_matrix_emp <- egk_long %>% 
    pivot_wider(id_cols = ind_code, names_from = NAME, values_from = coi_frac_ri_emp, values_fill = 0) %>% 
    select(-c(ind_code)) %>% 
    as.matrix()
```

```{r}
jr_matrix_estabs <- egk_long %>% 
    pivot_wider(id_cols = ind_code, names_from = NAME, values_from = coi_num_ri_estabs, values_fill = 0) %>% 
    select(-c(ind_code)) %>% 
    as.matrix()
```

```{r}
jr_matrix_emp <- egk_long %>% 
    pivot_wider(id_cols = ind_code, names_from = NAME, values_from = coi_num_ri_emp, values_fill = 0) %>% 
    select(-c(ind_code)) %>% 
    as.matrix()
```

We create two final matricies, one for employment and establishments. These are 229x229 sized matricies that capture the co-agglomeration betweeen two industries across the regions that they are present in. 

```{r}
coi_matrix_ij_estabs <- frac_ir_matrix_estabs %*% t(jr_matrix_estabs)
```

```{r}
coi_matrix_ij_emp <- frac_ir_matrix_emp %*% t(jr_matrix_emp)
```
...
# Create the reference 'sf' object
  ...
  results$state_all_estabs <- cbp_geog$tot_estab
  results$state_all_emp <- cbp_geog$tot_emp
  results$county_hhi_emp <- cbp_geog$county_hhi_emp
  results$county_hhi_estabs <- cbp_geog$county_hhi_estabs
  results$coi_num_ri_estabs <- cbp_geog$coi_num_ri_estabs
  results$coi_num_ri_emp <- cbp_geog$coi_num_ri_emp
  results$coi_denom_estabs <- cbp_geog$coi_denom_estabs
  results$coi_denom_emp <- cbp_geog$coi_denom_emp
...
```{r}
#GET AREA HHI FUNCTION

get_area_hhi <- function(data){ 
  
  area_results <- data %>% 
    filter(dist_mi <= geog_radius, !is.na(industry_code)) %>% 
    ungroup() %>% 
    mutate(area_ind_estabs = sum(ind_estabs), 
           area_ind_emp = sum(ind_emp), 
           area_perc_emp = ind_emp / area_ind_emp, 
           area_perc_estabs = ind_estabs / area_ind_estabs, 
           area_hhi_emp = 1 - sum(area_perc_emp**2, na.rm = TRUE), 
           area_hhi_estabs = 1 - sum(area_perc_estabs**2, na.rm = TRUE))
...
area_hhi_emp <- get_area_hhi(results) %>% 
  select(area_hhi_emp) %>% 
  distinct() %>% unlist() %>% unname() 

area_hhi_estabs <- get_area_hhi(results) %>% 
  select(area_hhi_estabs) %>% 
  distinct() %>% unlist() %>% unname() 
```
...
```{r}
#create a helper function to correct for 0 correlation
zero_norm <- function(matrix, hhi){ 
  ...
  #Get ratio of industries that have 0 correlation value in them to all industries present in the subset of industries within the geographical radius of the target county. Agglomeration should increase as the ratio of uncorrelated industries decreases
  matrix_ratio <- matrix_zeros / length(ind_list_index)
  
  #Find entries that have a zero-ratio, indicating that all industries are correlated
  zero_inputs <- which(matrix_ratio == 0)

  #Replace zero values with a value of 1
  matrix_ratio[zero_inputs] = 1
  
  #We need to weight the ratio of uncorrelated industries against the level of industry concentration in the county in general. 
...
```{r}
input_agg_emp <- final_matrix_calc(input_agg, results_ind_emp, area_hhi_emp)
input_agg_estabs <- final_matrix_calc(input_agg, results_ind_estabs, area_hhi_estabs)
```
...
```{r}
output_agg_emp <- final_matrix_calc(output_agg, results_ind_emp, area_hhi_emp)
output_agg_estabs <- final_matrix_calc(output_agg, results_ind_estabs, area_hhi_estabs)
```
...
```{r}
supply_agg_emp <- final_matrix_calc(supply_agg, results_ind_emp, area_hhi_emp)
supply_agg_estabs <- final_matrix_calc(supply_agg, results_ind_estabs, area_hhi_estabs)
```
...
for (i in 1:length(county_list)){
  ...
  county_hhi_emp <- results %>% 
  select(county_hhi_emp) %>% 
  distinct() %>% unlist() %>% unname() 

  county_hhi_estabs <- results %>% 
  ...
  input_emp <- final_matrix_calc(input_agg, results_ind_emp, 1)
  input_estab <- final_matrix_calc(input_agg, results_ind_estabs, 1)
  
  output_emp <- final_matrix_calc(output_agg, results_ind_emp, 1)
  output_estab <- final_matrix_calc(output_agg, results_ind_estabs, 1)
  
  supply_emp <- final_matrix_calc(supply_agg, results_ind_emp, 1)
  supply_estab <- final_matrix_calc(supply_agg, results_ind_estabs, 1)
